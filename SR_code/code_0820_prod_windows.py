...
# Based on code_0817_deploy; English comments only, logging via _log helper.
...
def main():
    ...
    def _run_sr_parallel(tiles, row_step):
        ...
        # ---- progress bar (terminal only, thread-safe) ----
        total = len(tiles)
        done = [0]
        prog_t0 = time.time()
        last_bucket = [-1]
        lock_prog = threading.Lock()

        def _show_progress():
            """Print one progress line per 5% step: [#####----] 40% 10/24 tiles ETA 85s."""
            d = done[0]
            bucket = (d * 100 // total) // 5
            if bucket == last_bucket[0]:
                return
            last_bucket[0] = bucket
            pct = bucket * 5
            bar_w = 40
            filled = d * bar_w // total
            bar = "#" * filled + "-" * (bar_w - filled)
            el = time.time() - prog_t0
            eta = el / d * (total - d) if d else 0.0
            print("\r[{:s}] {:3d}% {:>5d}/{:>5d} tiles  ETA {:6.1f}s".format(
                  bar, pct, d, total, eta), flush=True)
        ...
        acc = dict(sr=0.0, fill=0.0, pt=0.0,
                   slice=0.0, prep=0.0, fwd=0.0, sync=0.0, post=0.0)
        ran_espan = [False]  # flag: at least one espan/realesrgan batch ran
        ...
        def _worker(wid):
            ...
            while True:
                ...
                if ymlpath.find("restormer", 0, len(ymlpath)) > -1:
                    ...
                    # 5-stage timing (wall-clock, torch.cuda.synchronize-anchored):
                    #   slice / prep(CPU) / fwd(GPU) / sync(residual host wait) / post(D2H).
                    #   CUDA events unreliable here (mmsr_bundle owns stream mgmt, opaque;
                    #   measured negative elapsed) -> measure fwd via device-wide sync walls.
                    _ts = time.time()
                    ...
                    with lock_m:
                        with torch.cuda.stream(stream):
                            _tp0 = time.time()
                            t_in = torch.cat([util.get_LQ_tile(t, max_DN) for t in tiles],
                                             dim=0)
                            _tp1 = time.time()
                            torch.cuda.synchronize()   # drain prior work -> clean fwd base
                            _tf0 = time.time()         # fwd = submit + GPU busy + wait
                            m.feed_data_grid(t_in, need_GT=False)
                            m.test()
                            torch.cuda.synchronize()   # block until GPU actually finished
                            _tf1 = time.time()
                    _tw = time.time()
                    vis = m.get_current_visuals(need_GT=False)
                    ...
                    _tp = time.time()
                    with lock_t:
                        acc['slice'] += _tt - _ts       # host: wrap slicing
                        acc['prep']  += _tp1 - _tp0     # host: get_LQ_tile + cat
                        acc['fwd']   += _tf1 - _tf0     # GPU fwd (incl. async wait)
                        acc['sync']  += _tw - _tf1      # residual host wait after fwd sync (~0)
                        acc['post']  += _tp - _tw       # host: D2H + post-processing
                    ran_espan[0] = True
                ...
                # progress update after each batch (thread-safe)
                with lock_prog:
                    done[0] += len(items)
                    _show_progress()
        ...
        for t in threads:
            t.join()
        # clear the progress line so the timing summary starts on a fresh line
        print("\r" + " " * 80 + "\r", end="", flush=True)

        # per-stage SR infer profile (espan/realesrgan only; sync-anchored fwd)
        # _log -> also appended to SRLOG.txt (newline=True -> leading \\n in file)
        if ran_espan[0]:
            _sum = acc['slice'] + acc['prep'] + acc['fwd'] + acc['sync'] + acc['post']
            _log("[PROFILE] SR infer per-tile avg (s): "
                 "slice {:.4f} | prep {:.4f} | fwd-GPU {:.4f} | "
                 "sync {:.4f} | post {:.4f}  |  fwd {:.1f}% of infer stage".format(
                     acc['slice'] / total, acc['prep'] / total,
                     acc['fwd'] / total, acc['sync'] / total,
                     acc['post'] / total, 100.0 * acc['fwd'] / _sum if _sum else 0.0), opt)
