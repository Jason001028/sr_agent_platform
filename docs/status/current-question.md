...
**修法：加一份下划线同名件，不是把点号那份改名。** `app.py::_mirror_preview_name(jpg, source)`：
产物那份写盘（或命中缓存）之后 `copyfile` 成 `paths.drop_preview_path(product)`。**点号那份一个字
不动**，因为读判据全锚在它上面 —— 行的 `hasPreview`/`previewDiv`、静态 `jpgUrl`，以及前端
`lib/scene.ts::isBakedPreviewUrl`（`/\.preview\.jpe?g$/i`，据此决定拼 `?div=` 击穿 nginx 的
`max-age=3600`、以及换档位后要不要重烤）。改成下划线结尾会让烤出来的预览被当成「源本身就是显示件」，
档位与重烤两条逻辑同时失效。两条不变式：**不覆盖比缓存更新的同名件**（拖入链可能刚按别的档位烤过
它，判据 `mtime`）；**镜像失败只往 `preview_note` 尾巴加一句「同名（下划线）那份没落上：<原因>」**，
不改行的结论。缓存命中那一轮也补落一次，好让「只有点号那份」的老目录补齐。

**没改的**：惰性路径（打开某张图时的 `/preview`）**仍只写点号那份**。所以若用户当时看到的其实是
「打开未超分那张图之后多出来的 `PAN_NOSR.preview.jpg`」，那这份文件今天照样会出现 —— 它是服务端
自己那份缓存，不是烘焙链漏改。要把下划线名铺到惰性路径上是一行的事（在同一个写盘点调一次
`_mirror_preview_name`），但代价是**每次打开任何一张栅格都会往生产场景目录里多写一份文件**
（库列表点开每一景也会），这条线没我自作主张跨过去，等用户一句话。

**验证（开发机）**：后端 **663 passed / 4 skipped**（`TestProductPreviewBake` 27 例，本轮 +3：同一份
字节落两个名、更新的同名件不被覆盖、命中缓存那一轮把缺的下划线那份补上）。前端一行未动，故未重跑。
`flake8` 新增行零告警（该文件既有的 E501 未动）。

**待确认（真机）**：请 `ls -l <场景目录>` 一次，把带点的那个文件名原样贴回来 —— 要分清它是
`PAN_260318.preview.jpg`（急烤产物那份）还是 `PAN_NOSR.preview.jpg`（打开未超分那张图时惰性写的）。
两者的修法不同：前者本轮已修（下划线那份现在必定也在），后者要么接受、要么按上一段把镜像铺到
惰性路径。
