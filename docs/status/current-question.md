...
- **场景库「清除缓存」（09-22，开发机）**：表格卡片头部新增工具行 —— 左侧全选 +「已选 N 项」，
  右侧「清除选定」「全部清除」；清的是**盘阵上那份预览 JPG 缓存**（`<源 stem>_preview.jpg`），
  清完的行从列表移除（不重新检索、命中计数递减），库里的急烤状态记成 `cleared`、**不自动重烤**，
  下次打开惰性重烤。**这是全平台第一条删盘阵文件的端点**（`POST /api/scenes/clear-preview`），
  判据四道锁（名字 + **`srprev:` 规则戳** + 不是场景源 + 非符号链接）、只列用户点名的那个场景目录
  这一层；场景源与 `<编号>_mask.tif` 一律不碰（e2e 里逐个文件断「一个都没少」）。
  「全部清除」= **当前检索结果**（受筛选影响、到 `limit` 截断线为止），且要输入确认词才放行。
  三处「删了会被写回来 / 看起来没生效」都堵上了：急烤认领（CAS 写 `cleared`）、本地 blob 缓存
  （按 id 前缀失效）、**浏览器 HTTP 缓存**（本次真的重烤过时取静态图带 `cache:'no-store'` ——
  后一条顺带修好了「规则戳换代后最长一小时看到旧图」的既有洞）。结论四值里 `nothing` 与 `skipped`
  不混用（「一个同名件都没有」才叫 `nothing`；只有外来件时整条计 `skipped` 并留行在列表里，
  免得把还在盘上的文件冒充成「本来就没有」）。机制见
  [preview-bake-pipeline.md](../knowledge/preview-bake-pipeline.md) §4.12、契约 §3.9。
  验证：后端 **696 passed / 5 skipped**、前端 **308 passed** + `vue-tsc` 零错误 + `npm run build`、
  e2e `test-scenes.js` **112**（新增 `[J]` 节 33 条）/ `test-platform.js` 23 / `test-manual-scene.js` 197 全绿
  （`rows()` 顺手从「按 td 下标取列」改成按 CSS 类，免得下次加列时整体错位一位还照样「通过」）。
  **待真机确认（未解决）**：服务账号 `User=nginx` 对盘阵场景目录有没有**删除**权限 —— 没有时本功能
  只逐条报「失败 + 原因」，不会删掉任何东西。

- **场景库不再每次进入都自动重检索（09-22 当天续，开发机）**：用户报「场景库每次进入时都会重新自动
  检索，导致上次『全部清除』无效」。根因是 `ScenesPage` 每次挂载都无条件 `scenes.list()` —— 切到
  查看器再回来，上一轮清除的结果当场被抹掉（摘掉的行全回来、汇总行消失），看着像清除没生效；其实
  盘上那份删除**没有任何东西撤销它**（重新检索回来的行如实显示「未生成」）。现改成**只在本次会话
  第一次进本页时**检索（`stores/scenes.ensureSearched`，`searched` 在**发起**那一刻置位，失败也算），
  列表留在页面上直到用户自己按「检索」/「重置」——与清除那套「摘掉的行要回来得重新检索」同一口径。
  代价是列表可以是旧的（新落盘场景、别处刚烤的预览不反映），所以状态 chip 旁新增一行「上次检索
  HH:MM」如实标出这份数据的时刻。验证：前端 **380 passed** + `vue-tsc` 零错误 + `npm run build`、
  `test-scenes.js` **116 断言**（`[J]` 节新增 J6 四条：切走再回来不新增 `/api/scenes` 往返、被摘掉的
  行没自己回来、汇总行还在、chip 上有「上次检索」读数；**反向对照跑过** —— 把 `ensureSearched` 换回
  `list()`，J6 第一条当场红在「新增 1 次 /api/scenes」）、`test-manual-scene.js` 202 全绿。
  `test-platform.js` 当时停在 `[D]`「耗时列在场」——**已用基线（`git stash` 掉本轮改动后重跑）复现同一处
  失败**，与本轮无关；同日晚些时候再跑**整篇 26 条全绿**（本轮改动没碰过那条路径 —— `.qp-tbl td.elapsed`
  是队列页），所以它是**间歇性**的，不是稳定红项，留给下一轮定位。

- **「生成并打开」撞 404 的行改成灰色「已自动清理」（09-22 当天再续，开发机）**：用户报「生成并打开」
  鸡肋 —— 盘阵上的生产数据产出后约 10 天会被自动清理，而列表是「上次检索」那一刻的快照（上一条），
  于是**行还在、文件已经没了**，点下去只有「打开「X」失败：HTTP 404」，再点一次还是同一个 404。
  现改成：打开路径撞上 404 时把**这一行**标成 `purged`，那格的「打开」换成不可点的灰块「已自动清理」
  （预览列同口径改成「已清理」—— 与「已生成 / JPG 源」摆在一起是自相矛盾）。判据**只有 404**
  （`lib/api.ts::isSceneGone`，`http()` 改抛带 `status` 的 `HttpError`）：**422 刻意不算** —— 那是
  「源还在、只是烤不出来」（档位非法 / 多波段 / 条带读失败），拿它说「已自动清理」就是用一个猜的原因
  盖住真故障。标记**只活在前端这一次会话**（后端列表不带它：「试过一次撞上了」是前端的经历），重新检索
  整批换行时自然消失。错误行同时说清「盘阵上已没有这个文件（HTTP 404）…按『检索』刷新即知它还在不在」。
  验证：前端 **384 passed**（`api.test` 新增 4 条：404 认出来 / JSON detail 仍是 message / **422 不认** /
  网络层失败不认）+ `vue-tsc` 零错误 + `npm run build`、`test-scenes.js` **123 断言**（新增 `[K]` 节 6 条；
  H 段把刻意那一次 404 从「无浏览器错误」里放行、并断条数恰好为 1）；**反向对照跑过**（把 `isSceneGone`
  短路成 `false`，K 段当场红在「按钮=已自动清理」超时）、`test-manual-scene.js` **202 全绿**、
  `test-platform.js` **26 全绿**。
  **未覆盖的一路（下一轮）**：库行里**还没烤过预览**（按钮「生成并打开」）而源文件已被清掉时，前端先打
  `/preview`，后端回的是 **422「预览生成失败：场景文件不存在」** —— 这条路不会亮灰块。用户报的是 404，
  所以本轮没动后端；要覆盖它得让 `/preview` 在源文件不存在时回 **404**（而不是 422）—— 后端一行改动，
  需要重新部署（见 §4 该条「留作下一轮」）。

- **「打开」那一格按年龄推定灰块「已自动清除」（09-22 当天第三续，开发机）**：接上一条，用户嫌「撞一次
  才知道」太绕，要求更直白 ——「『生成并打开』统一换为『已自动清除』，99% 都是这种情况，实现越简单越好」。
  先摆两条从代码里读出来的事实，再让用户定口径：① 列表里的每一行在**检索那一刻**源文件都在
  （`is_scene_file` 要求 `p.is_file()`），所以「已清除」永远是**推定**、不是事实；② 平台只给**产物**急烤、
  **从不预烤输入影像**（`_bake_product_preview` 的注释：用户到底要不要看，打开之前无法知道），所以
  「盘上没有预览」恰恰是**还没被打开过的新景**，一律变灰会把「选景 → 打开 → 画掩码 → 提交 SR」这条
  主链路挡掉。**用户当天定的口径 = 按年龄分、阈值 3 天**。
  规则落在 `lib/scene.ts::presumedPurged(row, now)`（`PURGED_AGE_DAYS = 3`）：
  `row.purged || (不是 jpg 源 && 盘上一份预览都没有 && 年龄 > 3 天)` —— `row.purged` 就是上一条那个
  撞过 404 的实测标记，两半合起来才是「推定」的完整含义。年龄取**文件名里的日期**与 `rel` 目录里那个
  `年/月/日` 中**较年轻**的一个（重跑过的老影像文件名日期会骗人，目录日期兜底 —— 真机待核）。
  三条排除都写在函数上方：**档位不同不算**（只认「盘上一份预览都没有」，不看 `previewNeedsBake`，
  否则换一次档位整张表都变灰，而盘上那份就是刚烤的）、**新景不算**（上一条②）、**jpg 源行不算**
  （它被列出来本身就证明盘上那份 jpg 在，打开不需要烘焙）。日期读不出也不动它 —— 宁可让它能点。
  呈现随之收成两种：能点的按钮**一律写「打开」**（「生成并打开」这个文案消失，烤不烤是点下去之后的事），
  灰块写「已自动清除」并禁用；预览列同口径改「已清除」（与「已生成 / JPG 源」摆一起是自相矛盾）。
  **灰块是推定，就要有退路**：列表那一格已经不给点，粘场景路径到上方路径栏仍能打开老景 —— e2e 的
  `openViaPathBar` 钉住这条路，而且它同样会懒生成预览。反过来，**打开成功也是一条实证**：`open()` 成功
  路径末尾按 `lq_path` 把列表里同一景的那一行翻回来（`hasPreview=true` / `previewDiv=当前档位` /
  `purged=false`），不让列表继续印一个刚落空的结论；手工入口拿到的是另一份行对象，只能按场景目录对回去。
  上一条那个「老景没预览 + 文件已没了 → 后端 422」的缺口**被这一轮收窄**：这类行现在**根本不发请求**，
  剩下的只是「3 天以内的新景、文件已被清」（真机上仍可能碰到，后端那条 404 改造仍值得做）。
  验证：前端 **394 passed**（`scene.test` 新增 `presumedPurged` 11 条：边界 3 天不算 / 4 天算、档位不同
  不算、新景不算、jpg 源不算、两个日期取年轻的、文件名时间戳不被当成目录日期、日期读不出不算、
  `purged` 标记优先）+ `vue-tsc` 零错误 + `npm run build`；`test-scenes.js` **130 断言**（`[C]` 拆成
  C1 老景灰块不点不烤 / C2 当天新景照旧可点并走完懒生成 + 静态直读，`[F]` 与 `[J3]` 改走路径栏并在
  J3 断言「那一行当场翻回来」，`[D]`/`[J5]`/`[K]` 的标签文案同步；夹具新增一个**日期取跑测当天**的
  `FRESH_ROW`，写死日期过几天自己就变老景）+ `test-manual-scene.js` **202 全绿** +
  `test-platform.js` **26 全绿**（首跑一次间歇红在 `[D]`「耗时列给出终态耗时」，复跑全绿 ——
  与上一条记的是同一处间歇，未动过队列页）。
...
- E2E 测试：`.e2e/`（**2026-09-15 起入库**，只忽略 `node_modules/`+`fixtures/`+大图）——`test-vue-viewer.js` **190 断言**本地文件回归（含 G 段待修复清单、K 段对比绘制 + 右下角提醒）· `test-scenes.js` **130 断言**场景 http 打开（含 `[C]` 灰块口径 C1「老景不点不烤」/ C2「当天新景照旧可点并走完懒生成」+ `[F]`/`[J3]` 的**路径栏退路**、`[J]` 场景库清除缓存 33 条 + J6「切走再回来不重检索」4 条 + `[K]`「打开撞 404 → 已自动清除」6 条）· `test-platform.js` **26 断言** REST/SSE 全链路 + 布局（1600 视口实测页宽同宽 + 耗时列 nowrap；含**真后端 → 真 SSE → 右下角提醒**那条唯一真链路）· `test-manual-scene.js` **202 断言**盘阵任意场景目录（粘 `W:\…` 打开 → 画掩码 → 写盘阵 → 提交 SR，含 PAN 掩码命名与反推失败两条路；**G 段**粘单个 `.tif` 文件路径 → 预览按各边 1/2 烤进源图目录、且不能提交 SR；**H 段**《待修复清单》写回：导入 GBK 清单 → 粘路径 → 同步 → **从磁盘按字节读回**比对；**J/K 段**对比模式的窗口拖放与三类图芯片、后台预取边界；**L 段**左栏卡片带票拖进画布、拖中间产物 jpg 关联盘阵并挡住三处修复入口 —— 见 §4 该日条目）；跑法 `cd .e2e && node test-<name>.js`（前置 `cd frontend && npm run build`；puppeteer-core + 无界面 Chrome + 本地静态服务顶替 nginx + uvicorn 起真后端）。四个数字里 **26 / 202 是 2026-09-22 实测**（`test-platform.js` 3 条真链路断言 + 预取缓存那轮），
**130 是 2026-09-22「已自动清除」灰块那轮实测**（清除缓存那轮 112 → 不重检索 116 → 撞 404 那轮 123 → 本轮 +7），
本轮跑了 `test-scenes` / `test-manual-scene` / `test-platform` 三个，
`test-vue-viewer`（190）沿用当天更早的记录（更旧的值 58 / 65 / 22 / 197 都已过期，别照着用）
...
- [ ] **场景库「清除缓存」的写权限**（⏱10′，09-22 新增功能）· 场景库勾一个场景 →「清除选定」→ 确认；再点「全部清除」并输入确认词 `清除全部` · ✓= 明细里是 `cleared` 而不是 `failed: PermissionError`（**服务账号 `User=nginx` 对盘阵目录有没有 `unlink` 权限从没验过** —— 见 deploy/README 那两条「替换件属主变 nginx」的注意事项）；清掉的那行重新检索应显示「未生成」，点开重烤一次；顺带 `ls -l <场景目录>` 确认场景源 `.tif` 与 `<编号>_mask.tif` 一个没少 · 记录:
