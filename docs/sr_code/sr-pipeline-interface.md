...
> **版本**：v1.4 · **日期**：2026-08-17 · **适用对象**：`sr_agent_web` 后端与 agent 开发者
> **状态**：已定（v1.4，2026-08-17 实测校准）· **归档定位**：docs/sr_code/ 类目（SR_CODE 生产管线文档簇，随 SR_CODE 版本迭代更新）
>
...
| 退出码 | 含义 | 判定方式 |
|---|---|---|
| 0 | 情况较多，包含多种"进程正常结束但未完成任务"的情形：任务正常完成；云量超过阈值被过滤；配置文件缺失；输入 PAN 缺失；RC 步 PAN 尺寸不符。以上均表现为 `print` + `exit(0)` | 仅凭退出码无法区分，需结合输出产物判断（见下） |
| 1 | 运行期异常（进程输出带 traceback）：掩码路径指向目录、config.xml 标签缺失（IndexError）、`int("θ")` 转换失败（ValueError）、模型异常等 | 从进程输出或 SRLOG 中读取 traceback |
| 3 | GPU 相关异常：无 GPU 或 GPU 数量不等于 4 | 见 §7.1，部署前必须规避 |
...
| `CloudLimit` | 是 | 整数 | 云量阈值（百分比）。图像云量超过该值时，任务直接退出（退出码 0），不产生任何输出 |
...
| `Suffix` | 是 | 字符串 | 输出文件后缀。注意：标签存在但为空时，脚本读到的值为 None（见下），此时输出直接以输入名命名（输入已被先重命名为 `*_NOSR`，不会覆盖）。建议平台固定一个后缀，如 `<场景>_sr` |
...
- `GPUIDS`：部署脚本会读取（按任务选卡，实测值为 3）；本地版 `code_0817_prod.py` 不读取、硬编码 GPU 0。两者行为不一致，平台以部署脚本为准（§7.1）。
...
### 7.1 GPU 数量检查（部署前必须处理）

`code_0817_prod.py` 在入口处内置了生产队列守卫：
...
- 在 GPU 数量不为 4 的机器（含桌面端 RTX 3060 单卡）上原样部署，进程会直接以退出码 3 结束，并尝试执行 `systemctl stop slurmd.service`。
- 平台部署前须删除或注释该守卫（或改用不含该检查的脚本变体），契约涉及的其余功能不受影响。
- **本地版与部署脚本不一致（已确认）**：`code_0817_prod.py` 硬编码使用 GPU 0（模块级曾设为 `"1"`，`main()` 内覆盖为 `"0"`）；部署脚本则读取 `<GPUIDS>N</GPUIDS>` 选卡（实测模板值 3）。平台调用部署脚本时按任务指定 GPUIDS；GPU 数量守卫同样需要规避（见上）。
...
- **GPU 分配（依据部署脚本读取 GPUIDS）**：平台按任务分配 `GPUIDS`——开发服务器 4 卡可轮询 0–3，实现任务间并行；桌面端单卡固定为 0。单任务内部是 compute-bound，同一 GPU 上的并发没有收益，因此采用"任务间按 GPUIDS 分卡、任务内串行（任务队列）"。
...
| `--fail cloud` | 以退出码 0 结束，但不产生输出 tif（模拟云量过滤） |
...
| 文件 | 关键值 |
|---|---|
| `sfsr_config_test_espan2_cuda1.xml` | DatarootLQ=…/JXGF07A03_…_L1_PAN；GPUIDS=3（部署脚本会读取）；CloudLimit=80；DeleteOriTifNeeded=False（大写）；SRScale=2；Suffix=260318；OPT 指向 tile500 版 yml；MaskPath=…_mask.tif；MaskDilateFactor=2（部署脚本不读取） |
...
- `<GPUIDS>`：部署脚本会读取（按任务选卡）；本地版 `code_0817_prod.py` 不读取（硬编码 GPU 0）→ 平台以部署脚本为准（§9）。
- `<MaskDilateFactor>`：膨胀参数，0803 起已移除，本地与部署脚本均不读取。
