...
> **盘阵（阶段4）：浏览器不读盘阵 TIF，读服务器烘焙 JPG——后端稀疏采样映射/2% Linear/const 规则全镜像前端语义；scene id = base64url(rel)、路径白名单 realpath 校验拒 `../`/fake；jpg rec 的 stats 固定 0..255 让交互拉伸恒等；掩码换算走元数据 W/H。**
