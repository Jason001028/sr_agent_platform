...
- **判据只取能确证的那一个。** 只有 404 算「盘阵上没有这个文件」（`lib/api.ts::isSceneGone`）。
  **422 刻意不算** —— 那是「源还在、只是烤不出来」（档位非法 / 多波段 / 条带读失败），把它也说成
  「已自动清理」就是拿一个**猜的原因**盖住真实故障，用户会照着一个假结论去等它自己好。
  为了按状态码分派，`http()` 改抛带 `status` 的 `HttpError`（message 仍是给人看的那份文案）。
...
> **未超分那份的预览（阶段5，§4.11 那条链）：超分跑完的那一轮 tick 里顺带烤 `<场景目录>/PAN_NOSR.tif` → `<场景目录>/PAN_NOSR_preview.jpg`（档位取全局、命中判定同一个 `cache_hit`，落点直接用拖入链的 `<源 stem>_preview.jpg` 规则）；三处**故意**与产物那份不同 —— 不判沙箱（那份栅格是盘阵上的既有文件，跟这次跑在哪无关）、不动 `preview_state`/`preview_note`（那一列说的是产物，一个字段说不出两份文件的结局，只写盘 + 一行 stdout）、源名**钉死**（仓库 `writeTiff` 推出来的 `<产物 stem>_NOSR.tif` 与它对不上 —— 两个名字都试只会让「烤的是哪一份」说不清，先报 `skipped:` 让人看得见，真机 `ls` 一次就能分清）。**
