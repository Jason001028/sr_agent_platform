...
### 8.6 需求与精度取舍（2026-08-31 定 · 09-01 收口）

- **需求来源**：gui-requirements G1/P0 —— 在页面内交互式绘制 ROI 掩码，输出与原图**同分辨率**的
  `掩码.tif` 与 `掩膜中心点坐标.txt`。
- **核心矛盾**：预览与 JPG 都经过降采样与 8bit 亮度拉伸，而掩码需要全分辨率。
  JPG 的有损压缩与亮度拉伸**不影响掩码的几何形状**（掩码只关心某位置是否处理，即 0/1），
  **只有分辨率影响精度**。
- **设计演进**：最初计划把全分辨率栅格化放在 Python 侧（约 6 亿像素，当时怀疑浏览器无法承担）。
  2026-08-31 验证浏览器可以承担 —— 栅格化按行流式处理、配合 Deflate 压缩，内存占用只与压缩缓冲相关
  （见 §8.5）—— 于是**浏览器直出成为默认路径**，Python 路径保留为备选。两条路径的产物经逐像素
  交叉验证一致。
- **精度方案 = 混合**：在预览图上粗略绘制，再用 `thumbToOrig` 换算回全分辨率。
  2026-09-01 拍板**不做**「按可视区域读取全分辨率做放大精画」的升级（预览方案定为 JPG 中间产物，
  放弃网页内查看超过 1/3 细节的能力）。该决定只影响绘制时的便利性，不影响掩码产物本身的精度。
...
### 9.4 人工清除缓存：删得掉的是文件，删不掉的是缓存（2026-09-22）

场景库新增「清除选定 / 全部清除」（机制见 [preview-bake-pipeline.md](../knowledge/preview-bake-pipeline.md) §4.12）。
实现里真正的难点不是「怎么删文件」，是**删了之后用户凭什么能看见新的** —— 同一份预览在链路上
被缓存了三层，删盘上那个文件只算了第一层：

| 层 | 不清的后果 | 清法 |
|---|---|---|
| 盘上的 `<源 stem>_preview.jpg` | 就是这一层，删它 | `unlink`（判据四道锁） |
| **急烤认领**（`preview_state IS NULL` 的行会被重新烤） | 删完几秒后又冒出来 | 该目录所有 COMPLETED 行写 `cleared`（CAS，别用无条件 UPDATE） |
| **浏览器内存里那份 blob**（模块级单例，取图先查它） | 同一会话内点开还是旧字节 | 按 `scene id \|` 前缀失效 |
| **浏览器 HTTP 缓存**（nginx `max-age=3600`） | 用户以为清了个寂寞 | 本次真的重烤过时取静态图带 `cache:'no-store'` |

**最容易漏的是最后一条，而且它不是新洞。** 静态 URL 上那个 `?div=N` 一直被当成「击穿浏览器缓存」
的手段，但它的原理是**查询串变了 → 缓存键变了**：换档位有效，**档位没变而内容重烤**就完全无效。
后者有两条真实的路径 —— 规则戳换代（v2→v3，盘上原地重烤）与人工清缓存。也就是说这个洞在本轮
之前就存在（换包后最长一小时看到旧图），只是没人报过。**判据取「本次真的触发过烘焙」**
（`needBake`）：只有它成立时我们才**知道**服务端刚重写过那个文件，别的时候多绕一次缓存等于
每次打开都重下一遍。

**另一条教训来自 e2e**：`test-scenes.js::rows()` 原先是按 `tds[0..7]` 固定下标取列的。给表格加
首列勾选框（8 列 → 9 列）时，这类断言会**整体错位一位却照样「通过」** —— 取到隔壁列的文字、
与期望值比对失败时，报出来的位置和真正的原因差一整列。已改成按 `td` 上的 CSS 类取
（`td.c-sat` / `td.c-dims` / …）。加列前先数一数有没有人按位置在取：**位置是隐式契约**，
改结构时它不会报错，只会静静地说错话。

**用户当天就报了下一处：「每次进入场景库都自动重检索，上次『全部清除』无效」。** 查下来清除
本身**没有任何东西撤销它**（盘上那份 JPG 确实删了，重新检索回来的行也诚实地说「未生成」），
被撤销的是**用户看得见的那份证据** —— `onMounted` 无条件 `list()`，切走再回来就重检索一次，
被摘掉的行全回来、结果汇总行被清空。「我清了，回来一看全在」这个观察只能得出「没生效」。
所以凡是**把动作结果做成页面局部状态**的功能（这里 = 摘掉几行 + 一行汇总），都要顺带问一句
**这份状态活得过一次页面切换吗**：列表页的挂载自动刷新、抽屉收起、组件重建都会把它抹掉，
而用户分不清「状态被抹掉」和「动作没生效」。修法是让自动刷新**只发生在它不会覆盖用户结论的
时候**（本会话第一次进本页），代价明说 —— 列表可以是旧的，于是把「上次检索 HH:MM」摆在
页面上，让旧数据自己承认自己是旧的。

### 9.5 已经作废的操作：留一颗能点的按钮，等于让人反复撞同一堵墙（2026-09-22）

用户报「生成并打开」鸡肋：盘阵上的生产数据产出后约 10 天会被自动清理，而列表是**上次检索**
那一刻的快照（§9.4 那条改动让列表更长时间留在页面上），于是**行还在、文件已经没了**，点下去
只有「打开「X」失败：HTTP 404」，再点一次还是同一个 404。修法：打开路径撞上 404 时把这一行
标成 `purged`，那一格的「打开」换成**不可点的灰块**「已自动清理」。

三条口径值得记下来，它们是同一件事的三面：

- **判据只取能确证的那一个，而且要看是谁回的。** 只有**盘阵静态链**（`/disk-array/…`）上的 404 算
  「盘阵上没有这个文件」（`lib/api.ts::isSceneGone` = 404 **且** URL 落在 `/disk-array/`）。
  **422 刻意不算** —— 那是「源还在、只是烤不出来」（档位非法 / 多波段 / 条带读失败），把它也说成
  「已自动清除」就是拿一个**猜的原因**盖住真实故障，用户会照着一个假结论去等它自己好。
  **`/api/` 上的 404 同样不算**（响应体非 JSON ⇒ 这次请求没走到后端，见 `isProxyMiss`；这条 2026-09-24
  才补上 —— 当天正是它把「nginx 反代配错」读成了「文件已被清除」，见本节末）。为了按状态码与 URL
  分派，`http()` 改抛带 `status`/`url`/`jsonBody` 的 `HttpError`（message 仍是给人看的那份文案）。
- **标记是观测，不是事实，所以只活在前端这一次会话。** 后端列表不带 `purged`：「试过一次、
  撞上了」是前端的经历，盘上剩下的只是「取不到」。重新检索会把整批行换掉，那时标记自然消失
  （文件真没了，它也不再出现在列表里）—— 这正好防住「记住就好」变成遮羞布。
- **同一行里别摆两个互相矛盾的口径。** 预览列原先照旧显示「已生成 / JPG 源」，与旁边的
  「已自动清理」摆在一起是自相矛盾，所以那一列也加一个「已清理」态。

形态上仍是 `<button disabled>` 而不是 `<div>`：e2e 的 `rows()` 按 `td.c-act button` 读这一格的
文案，也按 `disabled` 判不可点（`clickRowButton` 会直接拒绝）—— 「不可交互」因此是**被断言**
的，不只是「看着像」。

**连带一条 e2e 教训**：`test-scenes.js` 的 `[J]` 段在重烤后立刻点「全部清除」，撞上一次
「按钮未找到或已禁用」——工具行的 `busy = clearing ‖ loading ‖ openingId`，而「已生成」标签
**比 `open()` 收尾早翻牌**（`fetchSceneJpg` 在取静态图之前就置了 `hasPreview`）。**等状态标签
不等于等这次操作收尾**：要接的下一步动作若依赖收尾后的状态，就得等那个状态本身（这里等按钮
从「打开中…」回到「打开」）。

**当天续（2026-09-22）：不用点就知道作废 —— 按年龄推定，但推定必须有退路。**
（⚠️ **这条口径 2026-09-24 已整条撤销**，理由与替代判据见本节末。以下保留原文以便追溯当时的取舍。）
用户嫌「撞一次才知道」太绕，要求更直白：「『生成并打开』统一换为『已自动清除』，99% 都是这种
情况，实现越简单越好」。**文案据此统一成「已自动清除」（预览列「已清除」）**——上面那半段里写的
「已自动清理」是当天早些时候的措辞，被这一轮换掉了；按钮也只剩两种取值：可点的「打开」与灰块
（「生成并打开」消失，烤不烤是点下去之后的事）。真按字面做（一律灰块）会**砸掉主链路**，两个从
代码里读出来的事实把它挡住了：

- **列表里每一行在检索那一刻源文件都在盘上**（`is_scene_file` 要 `p.is_file()`）——「已清除」
  永远是**推定**，不是事实；能确证的那一半只有「点过一次、撞了 404」。
- **平台只为产物急烤，从不预烤输入影像**（`_bake_product_preview`：用户要不要看在打开之前无法
  知道）——于是「盘上没有预览」恰恰是**还没被打开过的新景**的常态，一律变灰会把「选景 → 打开
  → 画掩码 → 提交 SR」当场挡住。

**口径最终由用户拍**：按年龄分，阈值 **3 天**（`presumedPurged` = `purged` ‖（非 jpg 源 ∧ 盘上
一份预览都没有 ∧ 年龄 > 3 天））。年龄取「文件名日期」与「`rel` 目录里的 年/月/日」中**较年轻**
的一个 —— 重跑过的老影像文件名日期会骗人，取年轻是保守方向。三条排除各有各的代价（换档位整表
变灰 / 挡住新景 / jpg 源本可直开），所以都写在函数上方，别当冗余删。

两条配套，缺一条口径就站不住：

- **推定必须留退路。** 灰块那一格不给点了，老景要打开只剩**粘场景路径到路径栏**这一条
  （`POST /api/scenes/resolve`，它同样会懒生成预览）。这条不是「顺便也支持」——它是这个口径的
  前提，所以 e2e 专门用 `openViaPathBar` 钉住（`[F]` 与 `[J3]` 都走它）。
- **打开成功要把它翻回来。** 成功打开 = 「这一景还在盘上」的实证，而且这一趟之后盘上必有它当前
  档位的预览。列表里同一景那一行得就地翻回可点（按 `lq_path`/场景目录对回去 —— 手工入口拿到的
  是另一份行对象，按对象身份找不到）。**别让列表继续印一个刚被证伪的结论。**

**按钮文案不再承担语义**：能点的一律「打开」（「生成并打开」消失），灰块写「已自动清除」；烤不烤
是点下去之后的事。副作用是 e2e 少了一个可等的收尾信号 —— 按钮在烘焙前后都叫「打开」，**先等这一趟
的产物（预览落盘），再等按钮解禁**，顺序不能反。

**夹具也要跟着「新」**：日期写死的夹具过几天自己就变成老景，C 段的前提当场失效。凡是断言「未生成 /
可打开」的夹具，日期都得取**跑测当天**（`FRESH_ROW`），并在用完后拆掉。

**2026-09-24：按年龄推定整条撤销 —— 推定不能拿来当判据。**

撤它的理由不是「不够准」，而是**它把一条走得通的路封掉了**。推定在**渲染时**就生效、不用点，而它依据的
「盘上没有预览」恰恰是**还没打开过的新景**的常态（平台只为产物急烤、从不预烤输入影像）。同一个事实，
09-22 拿它反推「多半是产出后被清掉了」，09-24 掉头看：这条反推等于把「选景 → 打开 → 画掩码 → 提交 SR」
这条主链路当成了例外。用户的口径回到最初那句 —— **取消按年龄推定，只认真撞过 404**。
`presumedPurged` / `PURGED_AGE_DAYS` / 那套「取较年轻的日期」全部删除；`SceneRow.purged` 只由打开时的
实测置位。

同一轮还挖出了**推定之外的另一半原因**，那才是这次线上故障的主因：nginx 的 `/api/` 下有个只为给
`/preview` 加 `Cache-Control` 而套的嵌套 location，**漏了 `proxy_pass`**。nginx 只选一个 location，
嵌套块**不继承** `proxy_pass`（final-location-handling 指令），但**继承** `proxy_set_header` /
`proxy_buffering` / `proxy_read_timeout` 这些普通指令 —— 于是那条 URL 落回 `root` 下的静态查找、由 nginx
自己回 **404 HTML**，字节根本没到后端。前端当时只看 `status === 404`，就把「请求没走到后端」读成了
「盘阵上文件已被清除」。两条合起来，就是用户看到的那一片灰。

- **判据要同时看「谁回的」**（上面那条 bullet 已按此改）：`isSceneGone` = 404 **且** URL 落在
  `/disk-array/`；新增 `isProxyMiss` = 响应非 JSON 且不是 `/disk-array/`（= 没走到后端）。走这一支时
  **如实报配置问题、不置 `purged`** —— 一个来路不明的状态码说明不了盘上有没有这一景。
- **同源的坑在部署侧**：别在 `/api/` 下顺手再套一处只加响应头的 location。真套了，子块里 `proxy_pass`
  必须**重写一遍**，且落盘后按 [deploy/README.md](../../deploy/README.md) §四 的 `/preview` 一条自查
  （`ct` 必须是 `application/json`，不能是 `text/html`）。
- **贴给内网机跑的补丁脚本要纯 ASCII + py2 兼容**：机器上裸 `python` 是 **2.7**，含中文注释的 heredoc
  直接 `SyntaxError`，**文件一个字都没写**；而紧随其后的 `nginx -t && nginx -s reload` 照样成功 ——
  差点把「没生效」读成「改了也没用」。写完先自查输出（例如打印替换计数）再 reload。

撤销后按钮仍只有两种：可点的「打开」（含「未生成」那种点下去才烤的）与**真撞过 404** 的灰块。
「粘场景路径」那条退路不再是灰块的唯一出口（灰块现在只出现在盘上真没了的时候），库外场景照旧走它。

### 9.6 名字的尾段：两处静默失败（2026-09-24，用户口径订正）

这轮做「未超分那份」（NOSR）时踩到两个同一个家族的坑：**都不报错，只是说错话**。两个都在
「名字怎么切尾段」这件事上。名字口径本身见
[preview-bake-pipeline.md](../knowledge/preview-bake-pipeline.md) §4.11。

**坑一：拿带分隔符的尾标记去比一段已经剥掉分隔符的字符串，恒不命中。**

判环节的函数拿到的是 `tail = stem[len(base)+1:]` —— 截取时**连那个分隔下划线一起切掉了**，
`PAN_NOSR` 的 `tail` 是 `NOSR`。于是 `tail.endswith("_nosr")` 永远为假，裸 `_NOSR` 尾被读成
「产物的 suffix 恰好叫 NOSR」。**界面上看起来是对的**：标签文字就是 `NOSR`（suffix 大写）。
错的是配色（产物那族 vs 未超分那族）、工具提示（「上一次超分产物」）、以及「能不能提交 SR」
这类下游判据。

教训不是「记得大小写」——是**判据里那个变量在哪一层**（原文件名 / 已剥尾段）必须写进文档或
函数名，带分隔符的常量只能跟**原文件名**比。这类错**断言标签文字抓不到**：这一轮是 e2e
把 `.stage` 的 **class**（`stage nosr`）也断言上，才把它钉住的。

**坑二：先在循环外预剥一次尾段，再进循环切，会把最该留下的那一条候选减掉。**

反推目录名的函数是「逐段切尾巴」的写法。原先在循环**外**先把 `_NOSR` 剥掉，再进循环切：
剥完剩下的正好就是目录名**真身**，而那个循环**恒要切至少一段** —— 真名一条候选都进不去，
于是拖入平台自己烤的那份 jpg 报「目录不存在」，**理由里列的还是两个被多切一段的假目录**。
修法是让它跟 `preview` 一样作为**尾段在循环内**被切（切到它就到头），而不是先剥。

教训：预剥与循环切是两套「减尾巴」机制，叠加起来会把唯一正确的那条减掉；**一个尾段只能由
其中一处负责**，另一个尾段（`preview`）当年就是靠「它在名单里、切到就停」走对的。

**顺带一条观察，回答「为什么同样的操作有时行有时不行」**：这两种名字的差别只在**有没有 14 位
成像时间戳**。带时间戳的（SC 场景 `<目录名>_NOSR_preview.jpg`）靠反推名字就能认出来；
不带时间戳的（RC 场景 `PAN_NOSR.jpg`）只能靠「同景的卡正开着」这个锚点 —— 这也是拖同景产物
jpg 一直以来的口径。所以「拖进去认得出 / 认不出」的分界不是浏览器、不是大小写，是名字里
有没有那条时间戳。
...
（订正，2026-09-22：这条规矩加了一个前提 —— **两格都占着才互换**。「另一格接住原来
那张」在目标格空着时没有东西可接，互换就退化成「把原来那格挖空」。见 §10.13。判据本身
已从 store 抽到 `lib/compare.placeInSplit`。）
...
`本体` —— 四十来字塞进标签没意义），产物用后端给的 `suffix` 大写，NOSR恒标 `NOSR`。
...
### 10.11 平台自己写下的文件，平台自己得认得（2026-09-21 真机反馈）

**(1) 两套名单各自都对，合起来漏掉的是自己。** `<stem>_preview.jpg` 带下划线是有理由的：
`is_scene_file` 白名单只认「文件名 == 目录名或 `PAN`」，带下划线才不会被收成一行场景。它同时
落在 `scene_search._NON_STAGE_TAILS`（挡云量图这类派生件）里。两条理由都成立，合起来的结果是
**平台烤进场景目录的文件，拖回来平台自己 404**。写文件的那一方得给读回的那一方留门，
「这份不是盘阵侧的派生件」和「这份不是场景显示件」是两个判断，不能共用一个名单。

**(2) 名单里能剥不能剥，判据是「剥掉之后会不会正好落到一个真实目录上」。** `cloud` / `thumb`
/ `mask` / `ori` 剥掉就落到真场景目录名上（云量图会被认成本体的显示件）—— 绝对不能剥；
`preview` 剥掉落到**那份栅格的 stem**（本体那份正好等于目录名），正是我自己写下的那份。
这条判据比「名字切得干不干净」硬得多，也是这条改动只放行一个尾段的理由。剥一层为止，
剥完仍在名单里（`<目录名>_cloud_preview`）照旧不认。

**(3) 404 别拿自己拼出来的路径当证据。** 真机上那屏「目录不存在」列的两条路径全是平台自己
拼的：模板 `<根>/{y}/{m}/{d}/{sat}/{mid}/{name}` 里 `{name}` 带着 `_preview`，而 `{mid}` 是
「丢掉景号那一段」拼出来的 —— 尾巴一脏，`{mid}` 一起脏，于是 `…_preview/…_preview` 两条
假路径摆在用户面前。用户没有任何办法从这一屏看出「目录是存在的，只是名字上多粘了七个字符」。

**(4) 夹具的层级跟真机不同，就测不出真机上「歪」的样子。** 上一轮我在扁平临时目录上验这条
404，结论「by design」是对的，但没走六层生产树模板，于是没看见报错会歪成那样。测试夹具是
六层真形态（`<根>/GSHC2IMPS/PRODUCT/{y}/{m}/{d}/{卫星型号}/{段级}/{景级}`），这类「模板一脏
就连带歪」的问题只有在这个形状上才会暴露。

### 10.12 「平台不猜目录」不等于「关联不上」：锚点是用户给的（2026-09-21 真机反馈）

**(1) 产物名按「输入影像名」拼，不是按目录名 —— 这个假设我写进了三处。** `SR_code/code_0817_prod.py`
是 `img_name[:-4] + "_" + suffix`。SC 场景的输入正好叫 `<目录名>.tif`，于是产物长得像
`<目录名>_sr.jpg`，我把这个形态当成了通则（文档、400 文案、`stage_of_jpg` 的入参名都按它写）。
RC 场景的输入是 `PAN.tif`（`util.check_sr_previous_step` 判 `<SolarAzimuth>` 为空），产物就是
**`PAN_<suffix>.jpg`**。用户真机上那个 `PAN_260318.jpg` 完全合规，是**我的格式写错了**。
教训不在这一条命名，而在推导方式：从**一个能解释得通的样例**推出一条通则，比读生成那行代码便宜，
但错得无声无息 —— 判断产物名该问「谁拼的它、拿什么拼的」，不是「我看过的那些长得像什么」。
落地上还有一档：产物名的基准是**那份被超分的影像的 stem**，一个目录里可能有多份输入候选
（`<目录名>.tif` 与 `PAN.tif` 并存），所以判定要**把各候选 stem 都试一遍**（`scene_search.stage_bases`），
不能钉死一个。`<目录名>` 只是**恰好在 SC 上成立**的那一个。

**(2) 「不猜目录」是「不拿名字去推目录」，不是「推不出来就别关联」。** `PAN_<suffix>.jpg` 里既没有
卫星段也没有成像时刻 —— 反推那一步只能 400，这条没错。但用户要的是**关联**，关联不等于反推：
盘阵上那个目录可以**由用户的上下文给定**（他当前打开着的就是那一景）。这不是猜，猜是从名字
编一个目录再 stat 上去；这里是用户自己的打开状态原样传下来，平台只是在**他指的那个目录里**
认这份图。守住「不猜」的正确姿势是补一条**别的来路的入口**，而不是把这类文件判成不可关联。

**(3) 锚点的判据仍是「同级栅格在」，而不是「名字对上目录名」。** 走上一条入口后最容易写歪的是
顺手复用 `_fingerprint_mismatch`（它比「jpg 名 == 场景目录名」）—— `PAN_260318.jpg` 恰恰不是目录名，
复用就等于把这条路重新堵死。真正该过的是 `stage_of_jpg` 的那道门：**这一环节自己的栅格躺在同级**。
两条路的判据本来就是同一个（都是「哪个环节」），差别只在**目录从哪来**。

**(4) 锚点是提示，不是断言 —— 于是它是软的。** 前端塞过来的是「最近显示过的场景 + A 格 + B 格」，
可能陈旧（用户刚关掉那一景）、可能重复。后端对坏值一律**跳过并把原因记进 400 的 detail**，
不 400 不 403，唯一硬的是白名单（越界目录直接丢）。反过来，**名字自己能反推时锚点连一次 stat
都不花** —— 否则老路径的探测量上限钉子全要重算。它在链路里的位置是**最后一道兜底**：能反推的
名字永远先按名字走，锚点只接那些名字里什么都没有的。

**(5) 一句文案说错，用户就会照着一个不存在的东西找。** 「能找到 `PAN_NOSR.jpg` 的中间产物」这类
说法（把 `.tif` 写成 `.jpg`、把「第二次跑才留得下」的机制说成必然）让人在目录里翻一个不存在的文件。
产物的第一份栅格在 `writeTiff` 里靠 rename 让位，**suffix 非空时第一次跑必然 FileNotFoundError 并
被吞掉**，`<产物名>_NOSR.tif` 只有同一 suffix 跑第二次才出现。说不准的时候宁可说「这份是什么、
为什么现在不该有」。

---

### 10.13 空目标格不做「互换」：把已经在另一侧的那张拖过来，另一侧不该变空（2026-09-22 用户反馈）

用户报：「刚进入"分屏查看"模式时，拖入某图到右侧…会将左图变为空」。

**(1) 定位靠的不是猜，是「谁能写这个字段」。** 全仓给 `paneA` 赋过值的只有三处：`placeRec`
的互换分支、`removeRec`（关掉那张图）、`setCompareMode`（进退分屏）。一次拖放既不关图也不切
模式 —— 于是只剩互换分支。它在「另一格已经是同一张」时写 `pane_other = cur`，而刚进分屏时
**目标格是空的（`cur === null`）**：没有一个「原来那张」可以接回去，「互换」退化成「把原来那格
挖空」。

**(2) 触发它只要「拖进来的这张 = 另一格那张」，两条路都到得了。** 一是点文件列表里
**正显示在另一侧**的那张卡片（§10.8 记的第二个入口）；二是拖一个**名字与字节数都与某条已有
rec 相同**的文件 —— `addFiles` 的 `dupOf` 会把它认成那条 rec、按它的 id 走，一条新 rec 都不开。
所以「我拖的是另一张图」与「左图变空」可以同时为真：用户在资源管理器里拖的是**文件**，平台看到
的是**那条 rec**。（§J 的注释里早就写着这个去重口径 —— 当时把它当成「用例会白写」的麻烦，
没当成「用户手上会发生的事」。）

**(3) 修法是给互换加一个前提，顺手把判据搬出 store。** 判据整段抽成 `lib/compare.placeInSplit`
（纯函数、可单测 —— 与 `dropSideAt` 同一类东西：都是「这一下拖放归谁」），互换那条改成
`if (otherId === id && cur !== null)`。目标格空着 → 照常落图、另一格不动（两格摆同一张）。
为什么是「两格同一张」而不是「什么都不做」：拖它过来的人要看的就是它出现在这一格；
**两格都占着时互换仍然保留**（09-21 定的口径：落点那半放新图、另一格接住原来那张）。空目标格
上另一条路只有「让一侧变空」，那正是用户报的那件事。

**(4) 验证要含反向对照。** 单测六例（`lib/__tests__/compare.test.ts`）：给落点 / 没给落点、
两格都占着时的互换、空目标格、落点那半本来就是它（`moved` 为假 —— 不重适配，用户的缩放留着）、
两格都空。e2e 新增 L2b 走**真拖动**（卡片自己填票 → 落点算出的 side='B'）。**先把判据换回旧写法
跑一遍**：L2b 当场红（`左格那张还在（null）`），换回新写法 197 项断言全绿 —— 少了这一步，
「这条用例钉住了没钉住」是看不出来的。

### 10.14 对比模式下画掩码：落点先减掉活动格的左上角（2026-09-22 用户要求）

用户要求「对比模式下也能绘制掩码」。原来 `enterDraw` 里挡着「对比模式只读」，理由是「分屏里
画到哪一格、写进哪一张 rec 都不明确」—— 这条理由在 `activeId`/`activeSide` 落地之后就不成立了：
活动侧本来就是「掩码 / ROI 统计 / 云量 / 任务状态跟随的那一张」，分屏里点哪半哪半就是活动侧，
掩码落点是**唯一**的。所以这次是**删掉那道门**，不是新加一套坐标。真正含糊的只剩一件事：
在本体影像上画、却在产物上落笔（掩码建在本体网格上，产物尺寸的掩码会把本体那份静默盖掉）——
这条由 `canDrawOn`（`rec.thumb` 存在 **且** 不是中间产物）挡住三处：`enterDraw`、绘制期间
`setActiveSide`（分屏里点产物那半，**出声说明**而不是静默忽略）、`onCanvasDownDraw` 兜底
（拖放落格 / 清单点选都能在绘制中途换掉活动侧）。

**真正动手改的是指针换算。** 分屏下每格的 `ViewState` 用的是**该格自己的局部坐标**（渲染时
`translate(rect.x, rect.y)`，见 §10.1），而 `mouseToThumb` 拿的是**画布**原点。`TifCanvas.mousePos`
原先直接把它俩对接 —— 在右格画掩码时，落点整体偏一个**左格宽**（再除以 scale）。这个错
不会表现为「画不出来」：框照常出现在指针附近（绘制层是按活动格裁 + 平移画的，数据错了画得
还对），所以肉眼与截图都看不出来，只有把 ROI 的坐标读出来比才现形。修法是换算前先减去
`activePaneRect()` 的左上角；单屏时它是 `{x:0,y:0}`，与改前逐字节相同。

**e2e 怎么钉住它**（`test-vue-viewer.js` K 段）：左格摆一张**产物**（用 `openSceneJpg` 注入合成
JPG，`stageKind:'product'`）、右格拖进本体，进分屏 → 真鼠标在**右格**画一个框 → 读 `getRois()`
比对**该格局部**的缩略图坐标（±1px）。判据取这个而不是「有没有框」：少减那一格宽的话 x 会落在
缩略图之外（200 宽的图上出现 800+），这条断言必红。另配两条反向对照：绘制层按活动格裁剪
（左半青绿像素必须为 0）、绘制中点半屏另一半时活动侧**不动**且给出说明、且**不会顺手落一个
框**（点下去那一下会走一次 `mousedown`，退化框靠 `commitRect` 的 `x1-x0 < 1` 丢掉）。

### 10.15 右下角任务提醒：挂在应用外壳上，不挂在页面上（2026-09-22 用户要求）

用户要求「在任务队列提交超分后切到查看器，完成时右下角用**非主题色**提醒」。三件事各有它的
理由，都不是随意选的：

- **挂 `App.vue`，不挂查看器页**。提醒的价值就在「人在别的页面干活时也能收到」；挂进任何单页
  都会随页面 unmount 一起消失。同理，队列 SSE 的订阅也要**常驻**（引用计数，见 api-contract
  §3.3）—— 原先它只在队列页挂载时或侧舱要活动场景时才连，用户切到查看器后这条流**根本没人
  订阅**，后端推什么前端都收不到。栈的状态也**不能**在离开查看器时清掉：那条提醒是用户在别的
  页面上唯一能看到的痕迹。
- **琥珀橙是刻意挑的非主题色**（令牌 `--notice-job*`，与 `--cmp-b` 同族但更亮一档）。青绿族
  （`--accent-*`）与深带绿是应用内一切常规元素的语言，而这条提醒的价值恰恰在于它「不是一个
  常规元素」；成功与失败**共用**这一套颜色，成败由标题文字说 —— 失败改涂 `--err` 会让同一件
  事看起来像两种东西。e2e 把这条要求写成可执行的：解析 `--notice-job` 的 computed 色值，
  断言橙条、浅底、深字三个都用它，且它与 `--accent-3`/`--accent-deep`/`--ok` **都不相等**
  —— 否则「非主题色」这条只是注释里的一句话，下一次配色调整就会悄悄失效。
- **成功条自动走（8s）、失败条常驻**，点击（整条或 ×）都去 `/queue`。失败条常驻是因为它带着
  原因（帧里的 `error` 优先，其次 `log_dir`，都没有才写「原因见队列页」），人可能不在屏前；
  成功条没有后续动作，留着只是占地方。「常驻」指的是**不会自己消失**，不是「点不掉」。
- **去重与「不补弹」**：键是 `<task_id>:<state>`，一次会话内有效（SSE 重连后服务端会补发真正
  变过的行，没有去重键同一条完成提醒会随每次重连再弹一遍）。刷新页面即清空 —— 刷新期间跑完
  的任务不补弹，不引入跨会话状态（用户口径）。
- **断线也要说出来**：这条流没有心跳帧（api-contract §3.3），掉线是**看不出来的**，不说的话
  用户只会以为「任务还没跑完」。掉线时栈里出一行灰条（灰色，不是橙 —— 它不是任务结果，只是
  「这段时间我看不见了」）。

---

> **锚点关联（阶段5，§10.12）：产物名按**输入影像名**拼（`code_0817_prod.py` 的 `img_name[:-4] + "_" + suffix`），SC 的输入是 `<目录名>.tif` 所以产物像 `<目录名>_sr.jpg`，RC 的输入是 `PAN.tif` 所以产物是 **`PAN_<suffix>.jpg`** —— 名字没错，错的是我把它当成了通则；判定基准要把该场景**所有**输入候选的 stem 都试一遍（`stage_bases`），不能钉死目录名。「平台不猜目录」是**不拿名字推目录**，不是「推不出来就别关联」：名字里没有卫星段/成像时刻的那类 jpg 走**用户上下文**给的 `anchor`（最近显示过的场景 → A 格 → B 格，上限 3 条），后端在他指的那个目录里认这份图 —— 判据仍是 `stage_of_jpg` 的「这一环节自己的栅格躺在同级」，**不能复用 `_fingerprint_mismatch`**（它比「jpg 名 == 目录名」，正是这类名字过不去的那条）。锚点是**提示不是断言**：坏值跳过、原因进 400 的 detail、只有白名单是硬的；名字自己能反推时它连一次 stat 都不花（老路径的探测量钉子不许动）。**
> **未超分那份（阶段5，§4.11 那条链）：名字 = **输入影像的 stem + `_NOSR`**（SC 即 `<目录名>_NOSR.tif`，RC 即 `PAN_NOSR.tif`），`writeTiff` 推出来的 `<产物 stem>_NOSR.tif` 退为**次选**、留在候选清单最后（两义都标 `NOSR` 标）；候选由 `scene_search.nosr_candidates()` **一处**产出，急烤那条链与 `/siblings` 读同一份 —— 09-21 那版在两处各写各的名字（一处硬编码 RC 名、一处只认次选），SC 场景两头都落空。**两处触发**：① 超分跑完那一轮 tick 里顺带烤（档位取全局、命中判定同一个 `cache_hit`、不判沙箱 —— 那份栅格是盘阵上的既有文件、不动 `preview_state`/`preview_note` —— 只写盘 + 一行 stdout，`skipped:` **报出试过哪些名字**）；② **拖入显示件**时前端对同景那一份做一次静默预热（命中才记账，盘上还没有那份时不锁死这一景）。落点仍是拖入链的 `<源栅格 stem>_preview.jpg` 规则，打开它时与打开任何产物同一套等待表示。**
> **平台自烤的预览（§10.11）：拖入链写进场景目录的 `<栅格 stem>_preview.jpg` 拖回来时按它代表的那份栅格认（`preview` 是 `_NON_STAGE_TAILS` 里唯一能剥的尾段，判据是「剥掉后落到自己写的栅格 stem 上」而不是「名字好切」；`cloud`/`thumb`/`mask`/`ori` 剥掉会落到真场景目录 → 一个都不许剥，只剥一层）；写文件的那一方要负责给读回的那一方留门 —— 「不是盘阵派生件」与「不是场景显示件」共用一个名单，就会漏掉自己写下的那份。**
...
> **面板里的读数得跟着面板里的按钮动（阶段5，2026-09-22）：设置浮层那行「本地预览缓存 N 项」曾经只在**打开浮层时**读一次快照，而能改这份缓存的开关（预取）就在那一行上面 —— 用户点开开关盯着紧挨着的数字，看到的永远是打开那一刻的值（走拖入链进来时那份缓存本来就是空的，于是**永远 0 项**），只能得出「预取没生效」。判据是：**同一个浮层里，一个控件的结果显示在另一个控件旁边时，读数必须订阅而不是快照**；做法上给缓存加了「内容真变了才响一次」的回调（`watchPreviewCache`），刻意不把 `stats()` 做成响应式 —— 那样每次取图都白重算一次渲染。同一浮层里还补了结果备注行：预取的三种收场（没可预取的 / 正在取 i/n / 已取到 N 项）都要说出来，否则静默的循环与「什么都没发生」在界面上无法区分。**
...
> **对比模式画掩码（阶段5，§10.14）：`enterDraw` 里那道「对比模式只读」的门已经删掉 —— 掩码就认活动侧（分屏里点哪半哪半是），落点是唯一的；留下的唯一一道门是 `canDrawOn`「有显示像素 + 是本体影像」（产物尺寸的掩码配本体 lq_path 会静默盖掉本体那份），三处共用：`enterDraw` / 绘制期间 `setActiveSide`（出声说明，不静默忽略）/ `onCanvasDownDraw` 兜底。**指针换算必须先减去活动格的左上角**：每格的 `ViewState` 是该格局部坐标，`mouseToThumb` 拿的是画布原点 —— 少减这一下，右格的落点整体偏一个左格宽，而框照常画在指针附近（绘制层按活动格裁 + 平移），肉眼看不出来，只有读出 ROI 坐标比才现形。**
> **右下角任务提醒（阶段5，§10.15）：栈挂 `App.vue`（不是查看器页）、队列 SSE 订阅常驻（引用计数）—— 提醒与它的数据源都不能随任何单页 unmount 消失；颜色用**非主题色**琥珀（`--notice-job*`，与青绿族刻意不同族），成败共用一套、由标题说；成功条 8s 自走、失败条常驻直到点掉（点整条或 × 都去 `/queue`）；去重键 `<task_id>:<state>` 仅一次会话有效（刷新不补弹）；SSE 没有心跳帧 → 掉线只能靠 `onOpen`/`onClose` 判出，掉线时栈里出一行灰条说明「提醒已断开」。**
...
> **预览名的两族与「再落一份」（阶段5，§4.9/§4.10）：同一份栅格有两条链各烤一份，名字只差一个字符 —— 缓存那份 `<stem>.preview.jpg`（点号，服务端自己读：行的 `hasPreview`/`previewDiv`、静态 `jpgUrl`，以及前端 `isBakedPreviewUrl` 的正则 `/\.preview\.jpe?g$/i`，它决定拼不拼 `?div=`、换档后要不要重烤）；给人看的那份 `<stem>_preview.jpg`（下划线，拖入链 / 未超分 / 急烤）。用户报「烘焙的名字把 `preview` 前的 `_` 输出成 `.`」时，全仓库**没有任何改写名字的代码**（`with_suffix` 的入参恒是源栅格）——那是两族名字在同一份栅格上各落各的。修法是**再落一份同字节件**（`copyfile`），不是把点号那份改名：改名等于服务端读不到自己刚烤的图、前端把烤出来的预览当成「源本身就是显示件」，档位与重烤两条逻辑同时失效。落之前比 `mtime`，别覆盖拖入链刚按别的档位烤的那份；镜像失败只加一句 note、不改行的结论。**（订正，2026-09-22：两族当天收口成一条 —— 全平台只有 `<源 stem>_preview.jpg` 一个名字（`paths.preview_jpg_name`），点号那份由 `paths.legacy_preview_path` 反推、被任何一条链碰到的场景目录顺手删；前端正则放宽成 `[_\.]preview\.jpe?g` 以认下还没被清掉的老目录。见 [preview-bake-pipeline.md](../knowledge/preview-bake-pipeline.md) §4.8。）**
> **落格与互换（阶段5，§10.13）：分屏下「这一张进哪一格」是判据 `lib/compare.placeInSplit` —— 给了落点进那一格、没给进活动侧、另一格已经是同一张则两格互换，**但两格都占着才互换**：目标格空着时「互换」没有「原来那张」可接，等于把另一格挖空（2026-09-22 用户报「刚进分屏把左图那张拖到右格、左图变空」）。触发它不要求用户拖的就是那一张卡片 —— `addFiles` 按**名字+字节数**去重，拖文件也会落到某条已有 rec 的 id 上。空目标格按「照常落图、另一格不动」处理。**
