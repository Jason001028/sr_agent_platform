...
`placeRec` 的互换分支（§10.7 的「两格互换」），实测能复现的入口只有两个：把**正在另一侧
...
**(2) 落位提示只对真带文件的拖放亮（实测）。** `onDragOver` 原来无条件
...
> **画面里的拖动（阶段5，§10.8）：画布区里唯一带可选文字的那层（解码遮罩）必须 `user-select: none` —— 漏了就是「拖一次选中文字、此后每次拖动都被 Chrome 当原生拖放」，落位提示「放在左侧/右侧」跟着乱亮、平移还一动不动；落位提示只对 `dataTransfer.types` 带 `Files` 的 dragover 亮（页面内拖放同样发 dragover 却什么都放不进来）；分隔线抓取带按下先不动，挪过 4px 再按**位移增量**改比例（按指针绝对位置会把按下点离中线那几像素先算进去）。**
