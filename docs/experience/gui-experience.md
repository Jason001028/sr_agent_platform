...
- **决策**：盘阵场景浏览器**不读原始 TIF**，改读服务器懒生成的 JPG（**09-17 起为各边 1/2 + 直方图均衡**；本条其余文字描述的 8192 长边 + 2% Linear 是当时规则，见 §9.1）。HttpSource/Range 方案废弃——本地 `tifDecode`/`sparse` 仍是浏览器读 TIF 的唯一路径（本地文件用），不与盘阵 JPG 混用。
- **服务端生成必须镜像前端稀疏语义**：预览/导出看起来要一致，就照抄前端映射——`ps=min(1,max_edge/max(W,H))`（**v2 起 `max_edge = round(max(W,H)*0.5)`，即各边 1/2**；旧规则是 8192 长边封顶）、`out(i,j)=src[round(i*(H-1)/(ph-1)), round(j*(W-1)/(pw-1))]`（端点对齐，杀右/下边条纹）；拉伸 **v2 起为直方图均衡**（旧为 2% Linear）+ WhiteIsZero 先反色；const 图规则与前端一致（全 0→黑，其他 const→128）。**同一组常量前后端各写一份，改任一端都要同步另一端的单测。**
...
> **盘阵（阶段4）：浏览器不读盘阵 TIF，读服务器烘焙 JPG（v2 = 各边 1/2 + 直方图均衡 + 规则戳写进 JPEG 注释）——后端稀疏采样映射/拉伸/const 规则全镜像前端语义；scene id = base64url(rel)、路径白名单 realpath 校验拒 `../`/fake；jpg rec 的 stats 固定 0..255 让交互拉伸恒等；掩码换算走元数据 W/H；裸 `.tif` 能看但父目录不是场景目录就不能提交 SR。**
