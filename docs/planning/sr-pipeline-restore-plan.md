...
>
> 动机有两条：① 每换一个场景就要手工拷一次，是**每次**的税，而不是**一次**的税；② SR 对 `DatarootLQ` 不是只读的（产物 tif、`Debug/` 日志、meta.xml 更新都写在那里，写产物前还会把输出路径上的同名旧文件 `os.rename(..., "_NOSR")`），"前端填路径"的字面含义就是"那个目录会被写入"——把只读保证交给人的记性，迟早会忘。
>
