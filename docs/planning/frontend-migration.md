...
| 5 · 平台新功能 | 聊天界面（Agent loop 对接 SSE）、共享任务队列（乐观更新 + 并发处理，服务端唯一事实源）、查看器掩码→SR 提交 | 契约先文档（`docs/planning/api-contract.md` 已定）；后端 190 + Vitest 114 + `.e2e/test-platform.js` 11 断言（离机 mock+fake）全过；真机见 current-question §6 | 完成（2026-09-02） |
