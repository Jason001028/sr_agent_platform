...
30. **Q：为什么 24739×24199 的 16bit 大图不能整幅解码进浏览器？（★ 项目高光）**
    **A：**
    ...
    - 两条并存路线：
      ...
      - **盘阵场景（阶段 4）**：浏览器干脆**不读原始 TIF 字节**，改读服务器预"烤"好的 8192 长边 JPG（稀疏采样+拉伸已做进像素），一刀把 2GB / 16384² 两个约束消掉。
    **追问：** 预览长边为何选 8192？→ 约是原图宽度的 1/3，够人眼判断；更大既费内存又逼近 canvas 上限。真要看细节时按需回源取局部（瓦片/重采样，见 Q19），概览常驻的始终是一份小内存。
...
- 数据源两套并存：本地文件走**稀疏条带读**；盘阵场景走**服务器预生成 8192 JPG**（浏览器不碰原 TIF 字节）。
