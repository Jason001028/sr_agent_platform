...
flowchart TD
    ...
    P1 --> BLOB["JPEG 字节"]
    P3 --> BLOB
    ROW -.->|"老景 + 没有预览 + 超过 3 天"| GONE["那一格的「打开」渲染成不可点的灰块「已自动清除」<br/>presumedPurged（2026-09-22）：不用点就按推定处理<br/>退路 = 粘路径走上面的 resolve 那条入口"]
    P1 -.->|"404"| GONE2["row.purged = true（2026-09-22）<br/>同一格、同一文案，但这一半是**点过的实证**<br/>列表是上次检索的快照、盘阵上的数据会被自动清除"]
    P3 -.->|"404"| GONE2
...
失败那条岔路（虚线）**只认 404**：那是「盘阵上没有这个文件」（静态 URL 由 nginx 直出，缺文件就是 404，非 JSON 体，
所以前端拿到的 message 只有裸的「HTTP 404」）。**422 不走这条路** —— 它是「源还在、只是烤不出来」，
照旧当错误报出来（拿一个猜的原因盖住真故障更糟）。细节见 §4.1 的 `purged`。

灰块有**两半**，落到同一格同一个文案上：一半是这里撞过的 404（实证），另一半是 `presumedPurged`
在渲染时按年龄**推定**（老景 + 盘上一份预览都没有 + 超过 3 天）—— 后者是多数情况（用户口径：
生产数据产出后放几天就被清掉），前者是兜住「列表比盘阵旧、但这一景还很新」的漏网行。
推定那一半必须留退路（粘路径 = 上面 resolve 那条入口），而且打开成功要把列表里同一景那一行翻回来。
...
| `purged` | **前端独有，后端不返回**（2026-09-22 新增）：打开这一行时撞了 404（盘阵上已经没有这个文件，多半是生产数据被自动清除）。标记后那一行的「打开」换成不可点的灰块「已自动清除」，预览列同口径显示「已清除」。**只活在前端这一次会话**，重新检索整批换行时自然消失 | 无（`stores/scenes.ts::open` 的 catch 里按 `lib/api.ts::isSceneGone` 置位） |

要点：`jpgUrl` 有值不等于缓存已存在，所以前端判断「这次会不会触发烘焙」用的是 `hasPreview`，不是 `jpgUrl` 是否为空。

**`purged` 的判据只有 404**（`isSceneGone`）：静态 `/disk-array/...` 上文件不在就是 404，而 422 是
「源还在、只是烤不出来」（档位非法 / 多波段 / 条带读失败）—— 把它也算成「已自动清除」就是拿一个**猜的
原因**盖住真故障。标记是**前端的经历**（试过一次、撞上了），不是盘上的事实，所以不进后端响应；
文件名对不上、文件被删、被挪走都长得一样，文案只陈述「取不到」，不写死原因。

`purged` 只是灰块的**一半**：另一半是渲染时的**推定**（`lib/scene.ts::presumedPurged`，2026-09-22）——
`purged` ‖（不是 jpg 源 ∧ 盘上一份预览都没有 ∧ 年龄 > `PURGED_AGE_DAYS` = 3 天），两半落到同一格同一个
文案上。推定那一半在**渲染**时就生效（不用点），依据是「平台只为产物急烤、从不预烤输入影像」这个事实的
反面：既没有预览、日期又很早，那多半是产出后被清掉了。三条排除各有各的代价（换档位整表变灰 / 挡住
「选景 → 打开 → 画掩码 → 提交 SR」主链路 / jpg 源本可直开），都写在函数上方。年龄取「文件名日期」与
「`rel` 目录里的年/月/日」中**较年轻**的一个，日期全读不出时不动它（宁可让它能点）。

推定必须有退路：灰块那一格不给点，**粘场景路径**（§3.2 的 resolve 入口）就成了打开老景的唯一入口；反过来，
**打开成功**（无论走哪条入口）按 `lq_path` 把列表里同一景那一行翻回来，不让列表继续印一个刚落空的结论。

**剩下的一条窄缝**：库行还没烤过预览**且日期在 3 天以内**、而源文件已被清掉时，`/preview` 回的是 422
而不是 404，这条路不会亮灰块（老的「老景没预览」那一类已被上面的推定覆盖，不再走这条缝）—— 要覆盖得让
后端在源文件不存在时回 404。详见 [timeline-archive.md](../status/timeline-archive.md) 该日条目。
...
### 4.11 顺带烤未超分那一份（2026-09-21，用户口径）

用户口径：**超分跑完就顺手把未超分那份的预览也烤出来** —— 「将 `PAN_NOSR.tif` 按照全局采样率
进行下采样，不考虑其他分支」。

实现是急烤那一轮里多一个函数（`app.py::_bake_nosr_preview`），跟在产物后面、同一个线程、
仍然串行：

| 项 | 值 |
|---|---|
| 源 | `<场景目录>/PAN_NOSR.tif`（`.tiff` 也认，两次 `is_file()` 封顶） |
| 落点 | `<场景目录>/PAN_NOSR_preview.jpg` —— 就是拖入链那条规则（`paths.drop_preview_path`：`<源 stem>_preview.jpg`） |
...
- **不动行的结论**：`preview_state` / `preview_note` 那一列描述的是**产物**预览，一个字段说不出
  ...
  状态四种（`baked:` / `cached:` / `skipped:` / `failed:`）。**没那份栅格时也照报** —— 名字对不对
  只有真机能证，静默跳过等于没人看得见。
- **名字钉死，不顺手多试一个**：仓库里 `SR_code/util.py::writeTiff` 的改名规则（改的是输出路径 →
  `<产物 stem>_NOSR.tif`，即 `PAN_260318_NOSR.tif`）**推不出** `PAN_NOSR.tif` 这个名字。两者对不上，
  真机 `ls` 一次就能分清；在代码里两个名字都试一遍，只会让「烤的到底是哪一份」说不清。
  待核项记在 [current-question.md](../status/current-question.md)。

谁**不**读这个文件（今天）：服务端不给它算 `hasPreview`（`/siblings` 的「NOSR」那一项找的是
