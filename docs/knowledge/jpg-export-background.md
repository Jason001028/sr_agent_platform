...
> 目标读者：对遥感影像和浏览器内部机制不熟悉、想理解 [utif-viewer.html](../../tif_viewer/utif-viewer.html) 这段新增流程的人。
...
> 每次成功读取一张 `.tif` 后，**自动**把当前"拉伸"显示效果下的画面，**重新用最高分辨率解码一遍**，
> 烘焙成一张高质量 JPG（长边默认 16384），**写到本地你选好的一个目录里**，
...
因为**预览图为了省内存被降采样到了长边 2048**，直接放大只会得到糊图。
...
> 预览用稀疏采样（快），导出 16384 也复用同一机制（因为这种结构的文件不需要金字塔，任意窗口都能字节切片）。
...
所以这里"尽可能保留细节"的取舍是：**分辨率尽量高（16384）+ 画质尽量好（q0.95）+ 拉伸模式可选**。
...
2. **导出 ≠ 预览**：预览是 2048 长边的；导出单独按 16384 重解一遍（`collectForExport()` 与预览同路由，只是目标长边不同）。
3. **带通累加器**：16384² 单波段若用全量 Float64 累加器 = 2.1GB > 2GB 上限 → 必须换成**按输出行带分段累加**（`bandPassCollect`），一段 256 行只占约 33MB。
4. **失败自动降档**：若 Edge 在 16384² 画布面积极限上拒绝（`RangeError/allocation` 等），自动把导出上限降到 **8192** 重试一次（`doExportJob` 的 catch 分支）。
...
结论：
- **单波段封顶 16384**——正好卡在画布面积上限，JS 分配也刚好在预算内（靠带通累加救回来）。
...
## 6. 对照源码（utif-viewer.html 关键函数定位）
...
- **为什么导出重新读一遍文件而不是复用预览？** 预览被降到 2048，细节不够；导出按 16384 重解一次（稀疏路径很便宜，分块路径会重新读文件，可接受）。
- **Edge 闪退/报 allocation 失败？** 那是 16384² 画布极限踩雷，程序会自动降 8192 重试；还不行就把 `EXPORT_BUDGET` 调小。
- **想改默认导出上限？** 改顶部常量 `JPG_MAX`（16384）即可，无需动其他逻辑。
